...
import prestashop.util.TimeoutRetryAnalyzer;
...
public class AddProductsToCartTest extends BaseTest {
    ...
    @Test(groups = TestGroup.RELEASE,
            testName = "E2E: User completes checkout with multiple products",
            retryAnalyzer = TimeoutRetryAnalyzer.class,
            description = "Verify that a user can add multiple products to the cart, complete checkout," +
                    " and see a successful order confirmation"
    )
    @Issue("E2E-02")
    public void shouldCompleteCheckoutSuccessfully() {
        ...
        step("Verify order confirmation", () -> {
            ...
            Assert.assertTrue(confirmationMessage.toLowerCase().contains("your order is confirmed"),
                    "Order confirmation message is missing or incorrect. Actual: " + confirmationMessage);
            Assert.assertEquals(productsCount, 2, "Expected 2 products in order, but found " + productsCount);
            Assert.assertTrue(hasMug, "Customizable mug is not present in order items");
            Assert.assertTrue(hasTShirt, "Hummingbird printed t-shirt is not present in order items");
            Assert.assertEquals(customizationText, "Best mug ever",
                    "Product customization text is incorrect. Expected: 'Best mug ever', but was: " + customizationText);
            Assert.assertEquals(subtotal, 39.62, 0.01, "Subtotal price is incorrect. Expected 39.62, but was " + subtotal);
            Assert.assertEquals(shipping, 8.40, 0.01, "Shipping price is incorrect. Expected 8.40, but was " + shipping);
            Assert.assertEquals(total, 48.02, 0.01, "Total price is incorrect. Expected 48.02, but was " + total);
            Assert.assertTrue(paymentMethod.contains("Cash on delivery"),
                    "Payment method is incorrect. Expected contains 'Cash on delivery', but was: " + paymentMethod);
            Assert.assertTrue(shippingMethod.contains("My carrier"),
                    "Shipping method is incorrect. Expected contains 'My carrier', but was: " + shippingMethod);
            Assert.assertFalse(orderReference.isBlank(), "Order reference should not be empty");
        });
    }
}